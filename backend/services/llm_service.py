...
def generate_literature_review(papers: list):
    serialized_papers = json.dumps(papers[:10], ensure_ascii=True)

    prompt = f"""
You are a research assistant.

Given multiple research papers, generate a proper literature review.
Do not write generic labels only. Use the paper content.
Summarize each paper separately, then synthesize them across the set.
...
{{
  ...
  "paper_insights": [
    {{
      "title": "paper title",
      "focus": "main problem or objective",
      "method": "method or approach used",
      "finding": "main result or contribution",
      "limitation": "one notable limitation or constraint"
    }}
  ],
  "synthesis": "...",
  "research_gaps": "...",
